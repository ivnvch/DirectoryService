...
public sealed class GetRootDepartmentsHandler : IQueryHandler<PaginationResponse<GetRootDepartmentDto>, GetRootDepartmentsQuery>
{
    ...
    {
        var key = "departments".ToCacheKey(
            query.Request.Pagination.Page, 
            query.Request.Pagination.PageSize, 
            query.Request.Prefetch!.Value);

        return await _cache.GetOrCreateAsync(key,
            factory: async _ => await GetRootDepartment(query, cancellationToken),
            tags: [CacheTags.Departments],
            cancellationToken: cancellationToken);
    }

    private async Task<Result<PaginationResponse<GetRootDepartmentDto>, Errors>> GetRootDepartment(
        GetRootDepartmentsQuery query,
        CancellationToken cancellationToken)
    {
