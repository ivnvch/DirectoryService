...
public sealed class GetDescendantsDepartmentHandler : IListQueryHandler<GetDescendantsDepartmentDto, GetDescendantsDepartmentQuery>
{
    ...
    public GetDescendantsDepartmentHandler(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var key = "departments".ToCacheKey(query.Pagination.Page, query.Pagination.PageSize, query.Id);

        return await _cache.GetOrCreateAsync(key,
            factory: async ct => await GetDescendantsDepartment(query, ct),
            tags: [CacheTags.Departments, CacheTags.ForDepartment(query.Id)],
            cancellationToken: cancellationToken);
    }

    private async Task<Result<List<GetDescendantsDepartmentDto>, Errors>> GetDescendantsDepartment(GetDescendantsDepartmentQuery query,
        CancellationToken cancellationToken)
    {
