...
public class SoftDeleteDepartmentTest : DirectoryBaseTests
{
    ...
    [Fact]
    public async Task Handle_WhenDepartmentDoesNotExist_ShouldReturnNotFoundError()
    {
        var nonExistentId = Guid.NewGuid();

        var result = await ExecuteSoftDeleteAsync(nonExistentId);

        Assert.True(result.IsFailure);
        Assert.Contains(result.Error, e =>
            e.Type == ErrorType.NOT_FOUND &&
            e.Messages.Any(m => m.Code == "department.not.found"));
    }

    [Fact]
    public async Task Handle_WhenLocationIsExclusiveToDepartment_ShouldSoftDeleteLocation()
    {
        var department = await CreateDepartmentAsync(locationsCount: 1);
        var exclusiveLocationId = department.Locations.Single().LocationId;

        var result = await ExecuteSoftDeleteAsync(department.Id);

        await ExecuteInDb(async context =>
        {
            var deletedLocation = await context.Locations
                .SingleOrDefaultAsync(x => x.Id == exclusiveLocationId, CancellationToken.None);
