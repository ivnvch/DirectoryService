    ...
    public async Task<Result<Guid, Errors>> Handle(UpdateDepartmentPathCommand command, CancellationToken cancellationToken)
    {
        ...
        if (command.ParentId.HasValue)
        {
            var parentDepartmentResult =
                await _departmentRepository.GetByIdWithLockAsync(command.ParentId.Value, cancellationToken);
            ...
            var isDescendants =
                await _departmentRepository.IsDescendantsAsync(department.Id, parentDepartment.Id, cancellationToken);
            if (isDescendants)
            {
                transactionScope.Rollback();
                return Error.Failure("department.not.descendant",
                    $"Cannot set a descendant department with Id {parentDepartment.Id} as parent.").ToErrors();
            }

            var setPathWithNewParent = department
                .UpdatePathWithParent(parentDepartment.Id, (short)parentDepartment.Depth,
                    parentDepartment.DepartmentPath);
        ...
        else
        {
            ...
            _logger.LogInformation("The department's path has been changed to the root path. All paths of the child divisions have been updated");
        }
